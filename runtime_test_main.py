"""
Contains modified functions and classes of main.py and alg_cluster.py
for running time testing.
"""
...
class ClusterTest:
    ...
    def merge_clusters(self, other_cluster):
        self.x = (self.x + other_cluster.x) / 2
        self.y = (self.y + other_cluster.y) / 2

    def distance(self, other_cluster):
        vert_dist = self.y - other_cluster.y
        horiz_dist = self.x - other_cluster.x
...
def closest_pair_strip(cluster_list, horiz_center, half_width):
    ...
    for idx in range(len(cluster_list)):
        if (abs(cluster_list[idx].x - horiz_center) < half_width):
            part_cl_idx.append(idx)

    part_cl_idx.sort(key = lambda idx: cluster_list[idx].y)
...
def fast_closest_pair(cluster_list):
    ...
    if (n_clusters <= 3):
        ...
        l_cluster_list.sort(key = lambda cluster: cluster.x)
        r_cluster_list.sort(key = lambda cluster: cluster.x)
        ...
        mid = (cluster_list[m_clusters - 1].x + cluster_list[m_clusters].x) / 2
